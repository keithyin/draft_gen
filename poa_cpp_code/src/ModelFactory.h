...
using NucleotideLevelFeats = PacBio::Data::NucleotideLevelFeats;
...
public:
    ...
    virtual std::unique_ptr<ModelConfig> Create(const NucleotideLevelFeats&) const = 0;
};
...
{
public:
    static std::unique_ptr<ModelConfig> Create(const std::string& name, const NucleotideLevelFeats&);
...
public:
    ...
    virtual std::unique_ptr<ModelConfig> Create(const NucleotideLevelFeats& feats) const
    {
        return std::make_unique<T>(feats);
    }
};
