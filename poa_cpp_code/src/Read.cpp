...
Read::Read(const std::string& name, const std::string& seq, const std::vector<uint8_t>& ipd,
    ...
    : Name{name}, Seq{seq}, IPD{ipd}, PulseWidth{pw}, SignalToNoise{snr}, Feats(feats), Model{std::move(model)}, CR{cr}
{
    if (ipd.size() != seq.size() || pw.size() != seq.size() || cr.size() != seq.size()) {
...
Read::Read(const std::string& name, const std::string& seq, const std::vector<uint8_t>& ipd,
    ...
    : Name{name}, Seq{seq}, IPD{ipd}, PulseWidth{pw}, SignalToNoise{snr}, Feats(feats), Model{std::move(model)}
{
    CR = std::vector<uint8_t>(seq.size(), 0);
