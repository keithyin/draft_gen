        ...
        std::vector<VD> PoaGraphImpl::consensusPath(AlignMode mode, int minCoverage) const
        {
            ...
            for (const VD v : sortedVerticesLocal)
            {
                ...
                for (const ED &e : inEdges(v, g_))
                {
                    ...
                    {
                        if (get(vertex_index, g_, v) < get(vertex_index, g_, bestVertex))
                            bestVertex = v;
                    }
                }
            }
...
#undef VERTEX_ON_PATH
        ...
        // vector<Consensus::ScoredMutation>* PoaGraphImpl::findPossibleVariants(
        ...
        //     auto* variants = new vector<Consensus::ScoredMutation>();
