...
static const std::string sam_DS{"IF"};
...
static const std::string sam_PU{"RN"};
...
static const std::string token_RT{"RECORDTYPE"};
static const std::string token_BK{"KITV"};
static const std::string token_SK{"CHV"};
static const std::string token_BV{"BCV"};
static const std::string token_FR{"FRAMERATEHZ"};
static const std::string token_CT{"CONTROL"};
static const std::string token_MODEL{"MODEL"};
...
static const std::string platformModelType_GSEQ{"Gseq"};
static const std::string platformModelType_GSEQ500{"Gseq500"};
static const std::string platformModelType_GSEQ1M{"Gseq1M"};
...
{
    switch (type) {
        case PlatformModelType::GSEQ1M     : return platformModelType_GSEQ1M;
        case PlatformModelType::GSEQ500     : return platformModelType_GSEQ500;
        case PlatformModelType::GSEQ   : return platformModelType_GSEQ;
        default:
...
{
    { platformModelType_GSEQ, PlatformModelType::GSEQ },
    { platformModelType_GSEQ500,     PlatformModelType::GSEQ500 },
    { platformModelType_GSEQ1M,  PlatformModelType::GSEQ1M },
};
// clang-format on
...
{
    if (nameToPlatformModel.find(name) != nameToPlatformModel.end()) {
        return nameToPlatformModel.at(name);
    } else {
        
        return PlatformModelType::GSEQ;
    }
}
...
void ReadGroupInfo::DecodeSamDescription(const std::string& description)
{
    ...
    for (const auto& token : tokens) {
        ...
        // 'mandatory' items
        // clang-format off
        if      (key == internal::token_RT) readType_ = std::move(value); //RECORDTYPE
        else if (key == internal::token_BK) bindingKit_ = std::move(value); // KITV
        else if (key == internal::token_BV) basecallerVersion_ = std::move(value); // BCV
        else if (key == internal::token_SK) sequencingKit_ = std::move(value); //CHV
        else if (key == internal::token_FR) frameRateHz_ = std::move(std::string("1000"));
        else if (key == internal::token_CT) control_ = (value == "TRUE");
        else if (key == internal::token_MODEL) model_ = std::move(value);
...
ReadGroupInfo ReadGroupInfo::FromSam(const std::string& sam)
{
    ...
    for (const auto& token : tokens) {
        ...
        // std::for_each(tokenTagUpper.begin(), tokenTagUpper.end(), toupper);

        // set read group info
        // clang-format off
        if      (tokenTagUpper == internal::sam_ID) rg.Id(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_CN) rg.SequencingCenter(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_DT) rg.Date(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_FO) rg.FlowOrder(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_KS) rg.KeySequence(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_LB) rg.Library(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_PG) rg.Programs(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_PI) rg.PredictedInsertSize(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_PU) rg.MovieName(std::move(tokenValue)); // RN run name for geneus
        else if (tokenTagUpper == internal::sam_SM) rg.Sample(std::move(tokenValue));
        else if (tokenTagUpper == internal::sam_DS) rg.DecodeSamDescription(std::move(tokenValue));  // IF , info for geneus
        else if (tokenTagUpper == internal::sam_PM) rg.PlatformModel(internal::PlatformModelFromName(std::move(tokenValue)));
        
        // clang-format on

        // if not platform name (always "PACBIO" for us), store as a custom tag
        else if (tokenTagUpper != internal::sam_PL)
            custom[tokenTagUpper] = std::move(tokenValue);
    }
                                                         ...
                                                         const std::string& basecallerVersion)
{
    ...
    for (const auto& row : internal::GetChemistryTableFromEnv()) {
        if (bindingKit == row[0] && sequencingKit == row[1] && basecallerVersion == row[2]) return row[3];
    }
