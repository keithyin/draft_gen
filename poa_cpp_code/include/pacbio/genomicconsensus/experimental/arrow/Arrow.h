...
struct Arrow
{
        ...
        const std::vector<PacBio::BAM::BamRecord>& reads, const ReferenceWindow& window)
    {
        ...
        for (const auto& r : reads) {
            ...
            const Data::NucleotideLevelFeats feats(r.BaseLevelCrs(), r.BaseLevelDwMeans(), r.BaseLevelDwVars());

            mappedReads.emplace_back(
                Read{r.FullName(), seq, ipd, pw, snr, feats, r.ReadGroup().SequencingChemistry()},
    ...
    static void LiftReads(const std::vector<int>& queryPositions,
                          std::vector<PacBio::Data::MappedRead>* const mappedReads)
    {
    ...
    static Consensus ConsensusForAlignments(const ReferenceWindow& window,
                                            ...
                                            std::vector<PacBio::BAM::BamRecord>* const readsUsed,
                                            ...
                                            const bool polish = true)
    {
        ...
        using Consensus = PacBio::GenomicConsensus::experimental::Consensus;
